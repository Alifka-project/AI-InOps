...
def safety_stock(demand_std: float, lead_time_days: float,
...
def reorder_point(avg_monthly_demand: float, lead_time_days: float,
                  ss: float) -> float:
...
def eoq(annual_demand: float, ordering_cost: float = 250.0,
        holding_cost_per_unit: float = 12.0) -> float:
    if annual_demand <= 0 or holding_cost_per_unit <= 0:
        return 0.0
    return math.sqrt(2 * annual_demand * ordering_cost / holding_cost_per_unit)


def inventory_policy(forecast_demand: float, demand_std: float,
                     lead_time_days: float, current_stock: float,
...
def hub_policies(hubs: pd.DataFrame, forecast_demand: float,
    ...
    for _, h in hubs.iterrows():
        ...
        p = inventory_policy(hub_demand, demand_std * share, avg_lead_time,
                             current, service_level)
        p["hub"] = h["hub"]
        rows.append(p)
    cols = ["hub", "current_stock", "safety_stock", "reorder_point",
            "eoq", "suggested_order", "status"]
